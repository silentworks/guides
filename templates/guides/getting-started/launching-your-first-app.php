...
<div class="left-col">
	...
	<div class="block grey steps">
		<ul>
			...
			<li><span class="guides-sprite step">10</span><a href="#step-10">Shutup and Deploy Already!</a></li>
		</ul>
	</div>
	<div class="block grey steps">
	...
	<p>Pagoda Box is a cloud based infrastructure so don’t assume your app clones will be on the same compute node. In fact, we try to spread them out as much as possible for redundancy. That means when we launch a clone of your app, we mount a read-only file system. Your app cannot write to the underlying filesystem. There are instances, albeit very limited, where you might need to write and share the data among nodes (this is most likely up-loadable media assets). Therefore, you need to determine which (if any) directories your app is going to need to be able to write to. Specify these directories.</p>
	...
	<div class="block red">
		...
		<p>You should also note that these directories are networked storage and at times may suffer from network latency. This won’t be a problem if you are only using these directories for media assets. Any other scenario is severely cautioned against.</p>
	</div>
	<h3 class="tag"><span class="guides-sprite cap"></span><span class="horizontal-guides-sprite title">YAML</span><span class="horizontal-guides-sprite green">&nbsp;&nbsp;&nbsp;</span><span class="guides-sprite green-end-cap"></span></h3>
	<div class="block grey code" id="default-box-config-settings">
		<script class='brush: yaml' type='syntaxhighlighter'>
	...
	<p>From time to time an app may crash. No worries, it happens to the best of us. In the unlikely event (because you are awesome) that it does happen, we would like to let you show your own custom error pages. It’s really simple. The example speaks for itself:</p>
	<h3 class="tag"><span class="guides-sprite cap"></span><span class="horizontal-guides-sprite title">YAML</span><span class="horizontal-guides-sprite green">&nbsp;&nbsp;&nbsp;</span><span class="guides-sprite green-end-cap"></span></h3>
	<div class="block grey code" id="default-box-config-settings">
		<script class='brush: yaml' type='syntaxhighlighter'>
			<![CDATA[
	...
	<p>Of course there are pieces of your app that you would like to keep from Peeping Tom. The locations directive allows you to restrict url’s by IP addresses.</p>
	<h3 class="tag"><span class="guides-sprite cap"></span><span class="horizontal-guides-sprite title">YAML</span><span class="horizontal-guides-sprite green">>&nbsp;&nbsp;&nbsp;</span><span class="guides-sprite green-end-cap"></span></h3>
	<div class="block grey code" id="default-box-config-settings">
		<script class='brush: yaml' type='syntaxhighlighter'>
			<![CDATA[
